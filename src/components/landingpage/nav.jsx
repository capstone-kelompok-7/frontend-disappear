  ...
  return (
    <div className="shadow-md w-full fixed top-0 left-0 z-10">
      <div className="md:flex items-center justify-between bg-white py-5 px-14">
        ...
        <ul className={`md:flex md:items-center gap-10 absolute md:static bg-white
         md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in py-2 text-center ${open ? 'top-20':'top-[-490px]'}` }>
          {menuItems.map((menuItem, index) => (
            <li key={index} className="md:my-0 my-4">
              <a href={menuItem.link} className="text-gray-400 hover:text-black duration-500">{menuItem.label}</a>
            </li>
          ))}
        </ul>
      </div>
